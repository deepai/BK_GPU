...
public:
		...
		StackElement *currentTopElement=new StackElement();
		StackElement *secondElement=new StackElement();

		StackElement *finalElement=new StackElement();

		std::vector<unsigned> *Psegment=new std::vector<unsigned>();
		std::vector<unsigned> *Xsegment=new std::vector<unsigned>();
		...
		void TestProcessPivot(BK_GPU::NeighbourGraph *Ng,BK_GPU::GPU_Stack *stack,Graph *host_graph,cudaStream_t &stream)
		{
			//#ifdef  TEST_ON
			{
				stack->topElement(currentTopElement,stream);
				...
				CudaError(cudaMemcpyAsync(Psegment->data(),Ng->data+currentTopElement->beginP,sizeof(unsigned)*currentTopElement->currPSize,cudaMemcpyDeviceToHost,stream));
				CudaError(cudaMemcpyAsync(Xsegment->data(),Ng->data+currentTopElement->beginX,sizeof(unsigned)*currentTopElement->currXSize,cudaMemcpyDeviceToHost,stream));
				...
				{
					unsigned currNode = Psegment->at(i); //Get Current Node ID
					...
					for(int j=0;j<currentTopElement->currPSize;j++)
					{
						...
						else
						{
							...
							if(std::binary_search(host_graph->columns.begin() + beginOffset, host_graph->columns.begin() + beginOffset + adjacencySize,Psegment->at(j)))
								numNeighbours++;
						}
					}
				...
				//pivot element
				unsigned pivot = Psegment->at(max_index);
				...
				std::vector<std::pair<unsigned,bool> > *auxillaryVec = new std::vector<std::pair<unsigned,bool> >(currentTopElement->currPSize);

				//ReArrange P values with Neighbors towards the left and sorted
				for(int i=0;i<currentTopElement->currPSize;i++)
				{
					bool isNeighbor=std::binary_search(host_graph->columns.begin() + beginOffset,host_graph->columns.begin() + beginOffset + adjacencySize,Psegment->at(i));
					auxillaryVec->insert(auxillaryVec->begin() + i,std::make_pair<int,bool>(Psegment->at(i),isNeighbor));
				}
				...
				for(int i=0;i<currentTopElement->currPSize; i++)
					Psegment->insert(Psegment->begin() + i, auxillaryVec->at(i).first);
				...
				{
					bool isNeighbor = std::binary_search(host_graph->columns.begin() + beginOffset, host_graph->columns.begin() + beginOffset + adjacencySize,Xsegment->at(i));
					auxillaryVec->insert(auxillaryVec->begin() + i ,std::make_pair<int,bool>(Xsegment->at(i),isNeighbor));
				...
				for(int i=0;i<currentTopElement->currXSize; i++)
					Xsegment->at(i) = auxillaryVec->at(i).first;
